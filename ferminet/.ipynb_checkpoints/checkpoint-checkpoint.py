...
def create_save_path(save_path: Optional[str]) -> str:
  ...
  # 这里修改为读取save_path路径，在该路径文件夹内建立符合timestamp的checkpoint保存文件夹。默认save_path为cpkt/psi
  if save_path is None or save_path == '':
    save_path = os.path.join(os.getcwd(), f'ckpt')
    if save_path and not os.path.isdir(save_path):
        os.makedirs(save_path)

  default_save_path = os.path.join(save_path, f'ferminet_{timestamp}')
  #default_save_path = os.path.join(os.getcwd(), f'ferminet_{timestamp}')
  ckpt_save_path = default_save_path
